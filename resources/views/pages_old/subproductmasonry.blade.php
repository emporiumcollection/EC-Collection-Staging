				...
				@foreach($parentArr as $parArr)
					@if(($parentArr[0])!=$parArr) 
						@if(end($parentArr)!=$parArr)
							<a href="{{ URL::to('subproductmasonry/'.$parArr->id) }}"><b>{{$parArr->name}} / </b></a>
						@else 
							{{$parArr->name}}
						@endif
					@endif
					...
					@if(!empty($catArr['child']))
						...
						@foreach($catArr['child'] as $subcat)
							@if($subcat['data']['file_type']=="folder")
								{{--*/ $breadlink = URL::to('subproduct/'.$subcat['data']['id']); /*--}}
							@else
						...
						@if(!empty($products['child']))
							...
							@foreach($products['child'] as $subproducts)
								...
								@if($subproducts['data']['file_type']=="folder")
									...
									$link = URL::to('subproductmasonry/'.$subproducts['data']['id']);
