...
<div class="container">
	@if(!empty($newspage_cat_produktion))
		<section data-selector="section" id="text-2col">
			<div class="container">
				<div class="row about-sec">
				
					<h1 data-selector="h3" class="title">{{\Lang::get('core.menu_frontend_produktion')}}</h1>
		...
		<section id="benefits-grid-images-left" class="new-padding">
			<div class="container">
				<div class="row no-dev">
					{{--*/ $pd=0; /*--}}
					@foreach($newspage_cat_produktion as $produktion)
						<div class="col-sm-4">
							<ul class="item-list overview">
								<li>
									{{--*/ $coverPic = ($produktion->featured_image_overview!='')? URL::to('uploads/article_imgs/'.$produktion->featured_image_overview): URL::to('uploads/images/product_no-image.jpg'); /*--}}
									
									{{--*/  $nwname = strtolower(str_replace(' ','-',$produktion->title_pos_1));
											$nwlink = URL::to('post/'.$nwname);
									/*--}}
									<a href="{{ $nwlink }}"><img src="{{$coverPic}}" alt="feature" class="screen img-circle"></a>
									<h3><a href="{{ $nwlink }}">{{ (\Session::get('newlang')=='English') ? $produktion->title_pos_1_eng : $produktion->title_pos_1}}</a></h3>
									<p class="editContent">
										{{--@if(strlen($produktion->description_pos_1) > 170)
											{!! (\Session::get('newlang')=='English') ? substr(strip_tags($produktion->description_pos_1_eng),0,170) : substr(strip_tags($produktion->description_pos_1),0,170).'...' !!}
										@else
										{!! (\Session::get('newlang')=='English') ? substr(strip_tags($produktion->description_pos_1_eng),0,-1) : substr(strip_tags($produktion->description_pos_1),0,-1) !!}
										@endif --}}
										
										{!! (\Session::get('newlang')=='English') ? strip_tags($produktion->description_pos_1_eng,'<span>') : strip_tags($produktion->description_pos_1,'<span>') !!}
									</p>
								</li>
							</ul>
						</div>
		...
		<section data-selector="section" id="text-2col">
			<div class="container2">
				<div class="row">
					<h3 class="poat-heading2">{{ (\Session::get('newlang')=='English') ? $newspage_cat_produktion[0]->cat_title_eng : $newspage_cat_produktion[0]->cat_title}}</h1>
					<div>{!! (\Session::get('newlang')=='English') ? $newspage_cat_produktion[0]->cat_description_eng : $newspage_cat_produktion[0]->cat_description !!}</div>
				</div>
			</div>
		</section>
	@endif 
