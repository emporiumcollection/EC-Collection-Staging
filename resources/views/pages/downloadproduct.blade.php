							...
							<div class="panel panel-default repeatVar1">
								...
								<div id="collapse1" class="panel-collapse collapse in" style="height: auto;">
									<div class="panel-body">
										@if(\Auth::check() == true)
											<a href="{{URL::to('downloadaszip/'.$prtfoldersname->id)}}"><i class="fa fa-download"></i> Alle Bilder (Lowres - Web - RGB)</a><br>
											<a href="{{URL::to('downloadhighresaszip/'.$prtfoldersname->id)}}"><i class="fa fa-download"></i> Alle Bilder (Highres - Print - CMYK)</a>
										@endif
										@if(!empty($pdf_fileDetail))
											@foreach($pdf_fileDetail as $pdf_files) 
												<br>
												@if ((strpos($pdf_files->file_name, 'deu.doc') !==false || strpos($pdf_files->file_name, 'eng.doc') !==false) && (\Auth::check() == true))
													<a href="{{$filepdfsrc . $pdf_files->file_name}}" title="Download" download="{{$pdf_files->file_name}}"><i class="fa fa-download"></i> 
													@if(strpos($pdf_files->file_name, 'deu.doc') !== false)
														{{($pdf_files->file_display_name!='')?$pdf_files->file_display_name:'Textdatei (DE)'}}
													@elseif(strpos($pdf_files->file_name, 'eng.doc') !== false)
														{{($pdf_files->file_display_name!='')?$pdf_files->file_display_name:'Textdatei (EN)'}}
													@endif
													</a>
												@elseif (strpos($pdf_files->file_name, 'pdf') !==false)
													<a href="{{$filepdfsrc . $pdf_files->file_name}}" title="Download" download="{{$pdf_files->file_name}}"><i class="fa fa-download"></i> 
														@if (strpos($pdf_files->file_name, 'pdf') !==false)
															{{($pdf_files->file_display_name!='')?$pdf_files->file_display_name:'Produktdatenblatt (PDF)'}}
														@endif
													</a>
												@endif
											@endforeach
						...
						@if(!empty($products['child']))
							...
							@foreach($products['child'] as $subproducts)
								...
								@if($subproducts['data']['file_type']=="folder")
									...
									$title = $subproducts['data']['title'];
									$descp = $subproducts['data']['description'];
									/*--}}
								@else
								...
								<div class="masonry-grid {{$algcnt}}">
									<img src="{{$ProductcoverPic}}" alt="feature" class="{{$custSize}}">
									<!--<div class="masonry-grid-title">
										{{$name}}
									</div>-->
									@if(\Auth::check() == true)
										<p><a href="{{ $downloadProductPicLow}}" title="Download" download="{{$name}}" target="_self"><i class="fa fa-download"></i> JPG Lowres</a></p>
										@if($subproducts['data']['downloadhigh']=='yes')
											<p><a href="{{ $downloadProductPicHigh}}" title="Download" download="{{$name}}" target="_self"><i class="fa fa-download"></i> JPG Highres</a></p>
										@endif
									@else
										<p><a href="#" data-toggle="modal" data-target="#myModal">{{(\Session::get('newlang')=='English') ? 'login to download' : 'einloggen zum herunterladen' }}</a></p>
									@endif
									
									@if($subproducts['data']['reserved']=='yes')
										<p>Reserved</p>
									@elseif($subproducts['data']['assign_lightbox']=='yes')
										<p>
											<a href="#" title="lightbox" rel="{{$subproducts['data']['id']}}" onclick="add_to_lightbox({{$subproducts['data']['id']}});">zur Lightbox hinzufügen</a>
										</p>
									@endif
							...
							<div class="single_lightbox_wrapper{{$lboxes->id}} single_lightbox_wrappertemp attached">
								...
								<div class="single_lightbox_inner_wrapper">
									<div class="single_lightbox_wrapper_left">
										...
										<div class="single_lightbox_controls">
											<a href="{{URL::to('lightbox_content_downloadpdf/'.$lboxes->id)}}" class="textButton default MrgLft10" > <i class="fa fa-download"></i> Download PDF</a>
											<a href="#" class="textButton asLightbox lightbox_email cboxElement MrgLft10" data-toggle="modal" data-target="#sendEmail" onclick="getlightbox($lboxes->id);"><i class="fa fa-envelope"></i> Email</a>
										</div>
									</div>
	...
	function add_to_lightbox(fileId)
	{
		if(fileId!="" && fileId > 0)
		{
			...
			if(tot_box>0)
			{
				...
				{
					alert('You can not add more than 3 products to Merkzettl.');
				}
			}
			else
			{
		...
		if(lightboxId > 0)
		{
			$.ajax({
			  ...
			  success: function(data){
				...
				{
					alert('Ihre Reservieren wir nun bearbeitet.');
				}
			  }
			});
		}
