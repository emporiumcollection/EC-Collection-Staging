...
<section data-selector="section" id="text-2col2">
	<div class="container">
		...
		<h1 data-selector="h3" class="title bjproduct-title">{{\Lang::get('core.menu_frontend_projekte')}}</h1>
...
<div class="container gg news-custom">
	@if(!empty($newspage_cat_projekten))
		<section id="benefits-grid-images-left">
			<div class="container">
				<div class="row no-dev">
				
					{{--*/ $nw = 0 /*--}}
					@foreach($newspage_cat_projekten as $project)
						{{--*/  $nwname = strtolower(str_replace(' ','-',$project->title_pos_1));
						...
						@if($nw<2)
							<div class="col-sm-6 MarBot10">
								<ul class="item-list2">
									<li>
										{{--*/ $Topnews = ($project->featured_image_top!='')? URL::to('uploads/article_imgs/'.$project->featured_image_top): URL::to('uploads/images/news_top_no-image.png');
										/*--}}
										<a href="{{$nwlink}}"><img src="{{$Topnews}}" alt="feature" class="screen img-circle" /></a>
										<h1><a href="{{$nwlink}}">
										{{-- @if(\Session::get('newlang')=='English')
											{{(strlen($project->title_pos_1_eng) > 255) ? substr($project->title_pos_1_eng,0,255).'...':$project->title_pos_1_eng}}
										@else
											{{(strlen($project->title_pos_1) > 255) ? substr($project->title_pos_1,0,255).'...':$project->title_pos_1}}
										@endif --}}
										
										{{ (\Session::get('newlang')=='English') ? $project->title_pos_1_eng : $project->title_pos_1 }}
										</a></h1>
										<p class="editContent">
										{{-- @if(\Session::get('newlang')=='English')
											{{(strlen(strip_tags($project->description_pos_1_eng)) > 200) ? substr(strip_tags($project->description_pos_1_eng),0,200).'...':strip_tags($project->description_pos_1_eng)}}
										@else
											{{(strlen(strip_tags($project->description_pos_1)) > 200) ? substr(strip_tags($project->description_pos_1),0,200).'...':strip_tags($project->description_pos_1)}}
										@endif --}}
										
										{!! (\Session::get('newlang')=='English') ? strip_tags($project->description_pos_1_eng, '<span>') : strip_tags($project->description_pos_1, '<span>') !!}
										</p>
										<h6><span class="flt_lt"> Post </span><span class="middleline">&nbsp;</span> <b>{{  date("d. m. Y", strtotime($project->publish_date)) }}</b></h6>
									</li>
								</ul>
							</div>
							@if($nw==1)
						...
						@else
							<div class="col-sm-3 img-box MarTop10 MarBot10">
								<ul class="item-list3">
									<li>
										{{--*/ $botnews = ($project->featured_image_bottom!='')? URL::to('uploads/article_imgs/'.$project->featured_image_bottom): URL::to('uploads/images/news_bot_no-image.png'); /*--}}
										<a href="{{$nwlink}}"><img src="{{$botnews}}" alt="feature" class="screen img-circle2"></a>
										<h6><span class="flt_lt"> Post </span><span class="middleline">&nbsp;</span> <b>{{  date("d. m. Y", strtotime($project->publish_date)) }}</b></h6>
										<h4><a href="{{$nwlink}}">
										{{-- @if(\Session::get('newlang')=='English')
											{{(strlen($project->title_pos_1_eng) > 50) ? substr($project->title_pos_1_eng,0,50).'...':$project->title_pos_1_eng}}
										@else
											{{(strlen($project->title_pos_1) > 50) ? substr($project->title_pos_1,0,50).'...':$project->title_pos_1}}
										@endif --}}
										...
										<p class="editContent">
										{{-- @if(\Session::get('newlang')=='English')
											{{(strlen(strip_tags($project->description_pos_1_eng)) > 235) ? substr(strip_tags($project->description_pos_1_eng),0,235).'...':strip_tags($project->description_pos_1_eng)}}
										@else
											{{(strlen(strip_tags($project->description_pos_1)) > 235) ? substr(strip_tags($project->description_pos_1),0,235).'...':strip_tags($project->description_pos_1)}}
										@endif --}}
		...
		<section data-selector="section" id="text-2col">
			<div class="container2">
				<div class="row">
					<h3 class="poat-heading2">{{ (\Session::get('newlang')=='English') ? $newspage_cat_projekten[0]->cat_title_eng : $newspage_cat_projekten[0]->cat_title}}</h1>
					<div>{!! (\Session::get('newlang')=='English') ? $newspage_cat_projekten[0]->cat_description_eng : $newspage_cat_projekten[0]->cat_description !!}</div>
				</div>
			</div>
		</section>
	@endif
