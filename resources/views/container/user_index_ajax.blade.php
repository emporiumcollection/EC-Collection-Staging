  ...
  <div class="page-content row">
								...
								<div class="row">
									{!! Form::open(array('url'=>'containersearch', 'class'=>'columns' ,'id' =>'search', 'method'=>'get' )) !!}
										...
										<div class="col-sm-6" style="text-align:right; padding-right:0px;">
											...
											@if((!empty($userpermissions) && $userpermissions->view==1) || (!empty($foldername) && $foldername->global_permission==1))
											<!-- View button -->
											<div class="btn-group">
								...
								@foreach($rowData as $row)
									...
									<div class="gallery-box">
										...
										<div class="caption {{$imgclass}}">
											<a href="{{ URL::to('files/view/').'/'.$fid.'/'.$row['id'].'?show='.$showType }}" title="{{$row['name']}}">{{strlen($row['name']) > 8 ? substr($row['name'],0,5)."~.".$expFile[1] : $row['name']}}</a>
										...
										<div class="thumb cinner{{$imgclass}}" <?php if($isImg==1) { ?> style="background: url('{{URL::to('uploads/thumbs/').'/thumb_'.$fid.'_'.$row['name']}}') no-repeat  center center; background-size:100px auto;" <?php } ?>>
											<a href="{{ URL::to('files/view/').'/'.$fid.'/'.$row['id'].'?show='.$showType }}" class="screenshot fancybox-buttons" rel="{{URL::to('uploads/thumbs/').'/medium_'.$row['name']}}" title="{{$row['name']}}" rel2="{{$fname}}" data-fancybox-group="button">
												&nbsp;
											</a>
										</div>
										<div class="info">
											...
											@else
												...
												<tr class="{{$alt}}">
													...
													<td class="rowtitle {{$imgclass}}">
														<a href="{{ URL::to('files/view/').'/'.$fid.'/'.$row['id'].'?show='.$showType }}" title="{{$row['name']}}">{{strlen($fname) > 22 ? substr($fname,0,18)."~.".$expFile[1] : $fname}}</a>
													</td>
	...
	<script>
		$(function(){
			...
			$("#enteremail").select2({
			  tags: [
			  <?php foreach($crmusers as $emailOBJ){
