...
{!! Form::open(array('url'=>'postarticle/delete/', 'class'=>'form-horizontal' ,'id' =>'SximoTable' )) !!}
		<select name='article_category' id='article_category' class="form-control" onchange="fetcharticlecategory(this.value);" > 
			<option value="">-Select-</option>
			@if(!empty($allcategories))
				@foreach($allcategories as $catlist)
					<option value="{{$catlist->cat_id}}" <?php echo ($curntcat == $catlist->cat_id) ? " selected='selected' " : '' ; ?>>{{$catlist->cat_name}}</option>
				@endforeach
			@endif
		</select>
...
function fetcharticlecategory(catg)
{
	window.location.href = "{{URL::to('postarticle')}}?selcat="+catg;
}
</script>		
@stop