...
@section('content')
														...
														<div class="foodemenu">
															...
															@if(!empty($barsArr[0]->datamenu))
																@foreach($barsArr[0]->datamenu as $resdatamenu)
																	<a href="{{$barsArr[0]->datamenupath.$resdatamenu->file_name}}" target="_self" download="{{$resdatamenu->file_name}}">{{$resdatamenu->file_display_name}}</a>
																@endforeach
															@endif
															</p>
														</div>
									 ...
									 <select name="restoid">
										 ...
										 @if(!empty($barsArr))
											  @foreach($barsArr as $bar)
												<option value="{{$bar->id}}">{{$bar->title}}</option>
											  @endforeach
										  @endif
									 </select>
								 </div> 
	...
	<section id="emotionSection" class="emotionSection">
	  ...
	  <div class="arrowsIcons">
		<a class="scrollpage" href="#bar1"><img src="images/arrow-up-icon.png" alt="icon"></a>
...
@section('sidebar')
    @include('frontend.themes.emporium.layouts.sections.bar_detail_sidebar')
@endsection
