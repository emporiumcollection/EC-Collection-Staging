...
<div class="sbox-tools" >
	...
	<div class="sbox-content"> 	
	    <div class="toolbar-line ">
			@if($access['is_add'] ==1)
	   		<a href="{{ URL::to('packages/update') }}" class="tips btn btn-sm btn-white"  title="{{ Lang::get('core.btn_create') }}">
			...
			@endif 
			<a href="{{ URL::to( 'packages/search') }}" class="btn btn-sm btn-white" onclick="SximoModal(this.href,'Advance Search'); return false;" ><i class="fa fa-search"></i> Search</a>				
			@if($access['is_excel'] ==1)
			<a href="{{ URL::to('packages/download?return='.$return) }}" class="tips btn btn-sm btn-white" title="{{ Lang::get('core.btn_download') }}">
	 ...
	 {!! Form::open(array('url'=>'packages/delete/', 'class'=>'form-horizontal' ,'id' =>'SximoTable' )) !!}
            ...
            @foreach ($rowData as $row)
                <tr>
				 ...
				 <td>
						...
						@endif
						@if($access['is_edit'] ==1)
						<a  href="{{ URL::to('packages/update/'.$row->id.'?return='.$return) }}" class="tips btn btn-xs btn-success" title="{{ Lang::get('core.btn_edit') }}"><i class="fa fa-edit "></i></a>
						@endif
...
$(document).ready(function(){
	...
	$('.do-quick-search').click(function(){
		$('#SximoTable').attr('action','{{ URL::to("packages/multisearch")}}');
		$('#SximoTable').submit();
	});
