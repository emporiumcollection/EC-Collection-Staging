    ...
    public function getPropertyByCategory(Request $request) {
       ...
       // $query .= "(SELECT tb_container_files.id FROM tb_properties_images  where tb_container_files.id=tb_properties_images.file_id and tb_properties_images.property_id=tb_properties.id and tb_properties_images.type='Property Images' Order By tb_container_files.file_sort_num ASC) as image";
        ...
        if (strtolower($request->slug) == 'yachts') {
            ...
            $OrderByQry .= " ORDER BY editor_choice_property desc, feature_property desc, (SELECT rack_rate FROM tb_properties_category_rooms_price WHERE tb_properties_category_rooms_price.property_id = tb_properties.id ORDER BY rack_rate DESC LIMIT 1) * 1 DESC LIMIT $start, $perPage ";
        }
