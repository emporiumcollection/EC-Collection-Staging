        ...
        if ($page != '') :
            ...
            if (count($content) >= 1) {
                ...
                if (file_exists($filename)) {
					...
					$this->data['landingads'] = \DB::table('tb_advertisement')->where('adv_type', 'sidebar')->where('adv_position', 'landing')->get();
					
					include(public_path() . '/revolution_slider/embed.php');
					
                    $this->data['slider'] = \DB::table('tb_sliders')->where('slider_category', 'Landing')->get();
                    $destts = array();
                    $maindest = \DB::table('tb_categories')->where('parent_category_id', 0)->get();
                    if (!empty($maindest)) {
                        $ctt = 0;
                        foreach ($maindest as $dest) {
                            if ($dest->id != 8) {
                                
                                $subdest = \DB::table('tb_categories')->where('parent_category_id', $dest->id)->get();
                                $getcats = '';
                                $chldIds = array();
                                if (!empty($subdest)) {
                    ...
                    $this->data['footer_text'] = \DB::table('tb_settings')->where('key_value', 'footer_text')->first();
                    $services_col1 = \DB::table('tb_settings')->where('key_value', 'services_column1')->first();
                    $services_col2 = \DB::table('tb_settings')->where('key_value', 'services_column2')->first();
                    $services_col3 = \DB::table('tb_settings')->where('key_value', 'services_column3')->first();

                    $landing_folder = \DB::table('tb_container')->where('name', 'landing')->first();
                    if (!empty($landing_folder)) {
                        $landing_slider_folder = \DB::table('tb_container')->where('name', 'slider')->where('parent_id', $landing_folder->id)->first();
                        if (!empty($landing_slider_folder)) {
                            $this->data['slider'] = \DB::table('tb_container_files')->where('folder_id', $landing_slider_folder->id)->get();
                            $this->data['sliderimgpath'] = (new ContainerController)->getThumbpath($landing_slider_folder->id);
                            $this->data['slidercontainerpath'] = (new ContainerController)->getContainerUserPath($landing_slider_folder->id);
                        }
                    }

                    $front_design = "grid";
                    $check_default_front_design = \DB::table('tb_settings')->where('key_value', 'frontend_design')->first();
                    if (!empty($check_default_front_design) && $check_default_front_design->content != "") {
                        $front_design = $check_default_front_design->content;
                    }
                    $this->data['default_front_design'] = $front_design;

                    $curdate = date('Y-m-d');
                    $frontend_folder = \DB::table('tb_landing_container')->join('tb_container', 'tb_container.id', '=', 'tb_landing_container.container_id')->select('tb_container.display_name', 'tb_container.title', 'tb_container.description', 'tb_container.cover_img', 'tb_container.temp_cover_img_masonry', 'tb_container.temp_cover_img', 'tb_container.created', 'tb_container.user_id', 'tb_container.display_name_eng', 'tb_container.title_eng', 'tb_container.description_eng', 'tb_landing_container.*')->orderBy('tb_container.sort_num', 'asc')->get();
                    $ffolder_Arr = array();
                    if (!empty($frontend_folder)) {
                        foreach ($frontend_folder as $frontend_folders) {
                            if ($frontend_folders->cover_img == "") {
                                if (!empty($check_default_front_design) && $check_default_front_design->content == "masonry") {
                                    $frontend_folders->cover_img = $frontend_folders->temp_cover_img_masonry;
                                } elseif (!empty($check_default_front_design) && $check_default_front_design->content == "grid") {
                                    $frontend_folders->cover_img = $frontend_folders->temp_cover_img;
                                } else {
                                    $frontend_folders->cover_img = $frontend_folders->cover_img;
                                }
                            } else {
                                $frontend_folders->cover_img = $frontend_folders->cover_img;
                            }
                            $ffolder_Arr[] = $frontend_folders;
                        }
                    }
                    $this->data['frontend_folder'] = $ffolder_Arr;

                    $this->data['frontend_projects'] = \DB::table('tb_post_projects')->where('status', 1)->where('publish_date', '<=', $curdate)->orderBy('tb_post_projects.publish_date', 'desc')->orderBy('tb_post_projects.id', 'desc')->first();

                    $this->data['frontend_cat_popup'] = \DB::table('tb_news_categories')->join('tb_post_articles', 'tb_post_articles.cat_id', '=', 'tb_news_categories.cat_id')->select('tb_post_articles.*')->where('tb_news_categories.cat_slug', 'popupstore')->where('tb_news_categories.cat_status', 1)->where('tb_post_articles.publish_date', '<=', $curdate)->where('tb_post_articles.status', 1)->orderBy('tb_post_articles.position_num', 'asc')->orderBy('tb_post_articles.publish_date', 'desc')->first();

                    $this->data['frontend_cat_messe'] = \DB::table('tb_news_categories')->join('tb_post_articles', 'tb_post_articles.cat_id', '=', 'tb_news_categories.cat_id')->select('tb_post_articles.*')->where('tb_news_categories.cat_slug', 'messe')->where('tb_news_categories.cat_status', 1)->where('tb_post_articles.publish_date', '<=', $curdate)->where('tb_post_articles.status', 1)->orderBy('tb_post_articles.position_num', 'asc')->orderBy('tb_post_articles.publish_date', 'desc')->first();

                    $this->data['frontend_cat_projekte'] = \DB::table('tb_news_categories')->join('tb_post_articles', 'tb_post_articles.cat_id', '=', 'tb_news_categories.cat_id')->select('tb_post_articles.*')->where('tb_news_categories.cat_slug', 'projekte')->where('tb_news_categories.cat_status', 1)->where('tb_post_articles.publish_date', '<=', $curdate)->where('tb_post_articles.status', 1)->orderBy('tb_post_articles.position_num', 'asc')->orderBy('tb_post_articles.publish_date', 'desc')->first();

                    $this->data['frontend_cat_newsletter'] = \DB::table('tb_news_categories')->join('tb_post_articles', 'tb_post_articles.cat_id', '=', 'tb_news_categories.cat_id')->select('tb_post_articles.*')->where('tb_news_categories.cat_slug', 'newsletter')->where('tb_news_categories.cat_status', 1)->where('tb_post_articles.publish_date', '<=', $curdate)->where('tb_post_articles.status', 1)->where('tb_post_articles.position_num', '>', 0)->orderBy('tb_post_articles.position_num', 'asc')->orderBy('tb_post_articles.publish_date', 'desc')->first();

                    $this->data['frontend_cat_pressesingle'] = \DB::table('tb_news_categories')->join('tb_post_articles', 'tb_post_articles.cat_id', '=', 'tb_news_categories.cat_id')->select('tb_post_articles.*')->where('tb_news_categories.cat_slug', 'presse')->where('tb_news_categories.cat_status', 1)->where('tb_post_articles.publish_date', '<=', $curdate)->where('tb_post_articles.status', 1)->where('tb_post_articles.position_num', '>', 0)->orderBy('tb_post_articles.position_num', 'asc')->orderBy('tb_post_articles.publish_date', 'desc')->first();

                    $this->data['frontend_cat_news'] = \DB::table('tb_news_categories')->join('tb_post_articles', 'tb_post_articles.cat_id', '=', 'tb_news_categories.cat_id')->select('tb_post_articles.*')->where('tb_news_categories.cat_slug', 'news')->where('tb_news_categories.cat_status', 1)->where('tb_post_articles.publish_date', '<=', $curdate)->where('tb_post_articles.status', 1)->where('tb_post_articles.position_num', '>', 0)->orderBy('tb_post_articles.position_num', 'asc')->orderBy('tb_post_articles.publish_date', 'desc')->first();

                    $this->data['frontend_cat_presse'] = \DB::table('tb_news_categories')->join('tb_post_articles', 'tb_post_articles.cat_id', '=', 'tb_news_categories.cat_id')->select('tb_post_articles.*')->where('tb_news_categories.cat_slug', 'presse')->where('tb_news_categories.cat_status', 1)->where('tb_post_articles.publish_date', '<=', $curdate)->where('tb_post_articles.status', 1)->where('tb_post_articles.position_num', '>', 0)->orderBy('tb_post_articles.position_num', 'asc')->orderBy('tb_post_articles.publish_date', 'desc')->take(3)->get();

                    if (!empty($services_col1) && $services_col1->content != '') {
                        $ser_col1 = explode(',', $services_col1->content);
                        $this->data['frontend_services1'] = \DB::table('tb_services')->where('status', 1)->whereIn('id', $ser_col1)->orderBy('id', 'desc')->get();
                    }

                    if (!empty($services_col2) && $services_col2->content != '') {
                        $ser_col2 = explode(',', $services_col2->content);
                        $this->data['frontend_services2'] = \DB::table('tb_services')->where('status', 1)->whereIn('id', $ser_col2)->orderBy('id', 'desc')->get();
                    }

                    if (!empty($services_col3) && $services_col3->content != '') {
                        $ser_col3 = explode(',', $services_col3->content);
                        $this->data['frontend_services3'] = \DB::table('tb_services')->where('status', 1)->whereIn('id', $ser_col3)->orderBy('id', 'desc')->get();
                    }

                    $this->data['projects'] = \DB::table('tb_post_projects')->where('status', 1)->where('publish_date', '<=', $curdate)->orderBy('tb_post_projects.publish_date', 'desc')->orderBy('tb_post_projects.id', 'desc')->get();

                    $this->data['designer'] = \DB::table('tb_designers')->where('designer_status', 1)->orderBy('id', 'desc')->first();

                    $this->data['designer_overview'] = \DB::table('tb_designers')->where('designer_status', 1)->orderBy('designer_num', 'asc')->get();

                    if (\Session::get('newlang') == 'English') {
                        $this->data['designer_title'] = \DB::table('tb_settings')->where('key_value', 'designer_title_eng')->first();
                        $this->data['designer_desc'] = \DB::table('tb_settings')->where('key_value', 'designer_desc_eng')->first();
                    } else {
                    ...
                    if (!empty($productgroup_folder)) {
                        ...
                        if (!empty($parent_query)) {
                            ...
                            foreach ($parent_query as $parent_obj) {
                                ...
                                $temp_array['parents'] = $parent_obj->parent_id;
                                $is_enabled = false;
                                ...
                                $temp_arr[$parent_obj->id] = $temp_array;
                            }

                            //print_r($this->data['enabled_parent_array']);
                    ...
                    $matgroup_folder = \DB::table('tb_container')->where('name', 'materialgruppen')->first();
                    if (!empty($matgroup_folder)) {
                        $par_folder = $matgroup_folder->id;
                        $parent_query = \DB::table('tb_container')->where('parent_id', $par_folder)->get();
                        $this->data['enabled_parent_array'] = array();
                        if (!empty($parent_query)) {
                            //echo '<pre>';
                        ...
                        $material_file_query = \DB::table('tb_container_files')->where('folder_id', $matgroup_folder->id)->orderBy('file_sort_num', 'asc')->first();
                        if (!empty($material_file_query)) {
                            $this->data['landing_material_file'] = $material_file_query;
                        }
                    }

                    $aktuellesArr = array('news', 'showrooms', 'popupstore', 'messe', 'projekten', 'video');
                    $this->data['newspage_cat_news'] = \DB::table('tb_news_categories')->join('tb_post_articles', 'tb_post_articles.cat_id', '=', 'tb_news_categories.cat_id')->join('tb_users', 'tb_users.id', '=', 'tb_post_articles.user_id')->select('tb_post_articles.*', 'tb_users.first_name', 'tb_users.last_name')->whereIn('tb_news_categories.cat_slug', $aktuellesArr)->where('tb_news_categories.cat_status', 1)->where('tb_post_articles.publish_date', '<=', $curdate)->where('tb_post_articles.status', 1)->orderBy('tb_post_articles.publish_date', 'desc')->orderBy('tb_post_articles.position_num', 'asc')->get();

                    $this->data['newspage_cat_Projekten'] = \DB::table('tb_news_categories')->join('tb_post_articles', 'tb_post_articles.cat_id', '=', 'tb_news_categories.cat_id')->join('tb_users', 'tb_users.id', '=', 'tb_post_articles.user_id')->select('tb_post_articles.*', 'tb_users.first_name', 'tb_users.last_name', 'tb_news_categories.cat_title', 'tb_news_categories.cat_description', 'tb_news_categories.cat_title_eng', 'tb_news_categories.cat_description_eng')->where('tb_news_categories.cat_slug', 'projekten')->where('tb_news_categories.cat_status', 1)->where('tb_post_articles.publish_date', '<=', $curdate)->where('tb_post_articles.status', 1)->orderBy('tb_post_articles.publish_date', 'desc')->orderBy('tb_post_articles.position_num', 'asc')->get();

                    $this->data['newspage_cat_Messe'] = \DB::table('tb_news_categories')->join('tb_post_articles', 'tb_post_articles.cat_id', '=', 'tb_news_categories.cat_id')->join('tb_users', 'tb_users.id', '=', 'tb_post_articles.user_id')->select('tb_post_articles.*', 'tb_users.first_name', 'tb_users.last_name', 'tb_news_categories.cat_title', 'tb_news_categories.cat_description', 'tb_news_categories.cat_title_eng', 'tb_news_categories.cat_description_eng')->where('tb_news_categories.cat_slug', 'messe')->where('tb_news_categories.cat_status', 1)->where('tb_post_articles.publish_date', '<=', $curdate)->where('tb_post_articles.status', 1)->orderBy('tb_post_articles.publish_date', 'desc')->orderBy('tb_post_articles.position_num', 'asc')->get();

                    $this->data['newspage_cat_PopUpStores'] = \DB::table('tb_news_categories')->join('tb_post_articles', 'tb_post_articles.cat_id', '=', 'tb_news_categories.cat_id')->join('tb_users', 'tb_users.id', '=', 'tb_post_articles.user_id')->select('tb_post_articles.*', 'tb_users.first_name', 'tb_users.last_name', 'tb_news_categories.cat_title', 'tb_news_categories.cat_description', 'tb_news_categories.cat_title_eng', 'tb_news_categories.cat_description_eng')->where('tb_news_categories.cat_slug', 'popupstore')->where('tb_news_categories.cat_status', 1)->where('tb_post_articles.publish_date', '<=', $curdate)->where('tb_post_articles.status', 1)->orderBy('tb_post_articles.publish_date', 'desc')->orderBy('tb_post_articles.position_num', 'asc')->get();

                    $this->data['newspage_cat_Showrooms'] = \DB::table('tb_news_categories')->join('tb_post_articles', 'tb_post_articles.cat_id', '=', 'tb_news_categories.cat_id')->join('tb_users', 'tb_users.id', '=', 'tb_post_articles.user_id')->select('tb_post_articles.*', 'tb_users.first_name', 'tb_users.last_name', 'tb_news_categories.cat_title', 'tb_news_categories.cat_description', 'tb_news_categories.cat_title_eng', 'tb_news_categories.cat_description_eng')->where('tb_news_categories.cat_slug', 'showrooms')->where('tb_news_categories.cat_status', 1)->where('tb_post_articles.publish_date', '<=', $curdate)->where('tb_post_articles.status', 1)->orderBy('tb_post_articles.publish_date', 'desc')->orderBy('tb_post_articles.position_num', 'asc')->get();

                    $this->data['newspage_cat_Blog'] = \DB::table('tb_news_categories')->join('tb_post_articles', 'tb_post_articles.cat_id', '=', 'tb_news_categories.cat_id')->join('tb_users', 'tb_users.id', '=', 'tb_post_articles.user_id')->select('tb_post_articles.*', 'tb_users.first_name', 'tb_users.last_name')->where('tb_news_categories.cat_slug', 'blog')->where('tb_news_categories.cat_status', 1)->where('tb_post_articles.publish_date', '<=', $curdate)->where('tb_post_articles.status', 1)->orderBy('tb_post_articles.publish_date', 'desc')->orderBy('tb_post_articles.position_num', 'asc')->get();

                    $this->data['newspage_cat_Newsletter'] = \DB::table('tb_news_categories')->join('tb_post_articles', 'tb_post_articles.cat_id', '=', 'tb_news_categories.cat_id')->join('tb_users', 'tb_users.id', '=', 'tb_post_articles.user_id')->select('tb_post_articles.*', 'tb_users.first_name', 'tb_users.last_name')->where('tb_news_categories.cat_slug', 'newsletter')->where('tb_news_categories.cat_status', 1)->where('tb_post_articles.publish_date', '<=', $curdate)->where('tb_post_articles.status', 1)->orderBy('tb_post_articles.publish_date', 'desc')->orderBy('tb_post_articles.position_num', 'asc')->get();

                    $this->data['newspage_cat_team'] = \DB::table('tb_news_categories')->join('tb_post_articles', 'tb_post_articles.cat_id', '=', 'tb_news_categories.cat_id')->join('tb_users', 'tb_users.id', '=', 'tb_post_articles.user_id')->select('tb_post_articles.*', 'tb_users.first_name', 'tb_users.last_name', 'tb_news_categories.cat_title', 'tb_news_categories.cat_description', 'tb_news_categories.cat_title_eng', 'tb_news_categories.cat_description_eng')->where('tb_news_categories.cat_slug', 'team')->where('tb_news_categories.cat_status', 1)->where('tb_post_articles.publish_date', '<=', $curdate)->where('tb_post_articles.status', 1)->orderBy('tb_post_articles.publish_date', 'desc')->orderBy('tb_post_articles.position_num', 'asc')->first();

                    $this->data['newspage_cat_partner'] = \DB::table('tb_news_categories')->join('tb_post_articles', 'tb_post_articles.cat_id', '=', 'tb_news_categories.cat_id')->join('tb_users', 'tb_users.id', '=', 'tb_post_articles.user_id')->select('tb_post_articles.*', 'tb_users.first_name', 'tb_users.last_name', 'tb_news_categories.cat_title', 'tb_news_categories.cat_description', 'tb_news_categories.cat_title_eng', 'tb_news_categories.cat_description_eng')->where('tb_news_categories.cat_slug', 'partner')->where('tb_news_categories.cat_status', 1)->where('tb_post_articles.publish_date', '<=', $curdate)->where('tb_post_articles.status', 1)->orderBy('tb_post_articles.publish_date', 'desc')->orderBy('tb_post_articles.position_num', 'asc')->get();

                    $this->data['newspage_cat_produktion'] = \DB::table('tb_news_categories')->join('tb_post_articles', 'tb_post_articles.cat_id', '=', 'tb_news_categories.cat_id')->join('tb_users', 'tb_users.id', '=', 'tb_post_articles.user_id')->select('tb_post_articles.*', 'tb_users.first_name', 'tb_users.last_name', 'tb_news_categories.cat_title', 'tb_news_categories.cat_description', 'tb_news_categories.cat_title_eng', 'tb_news_categories.cat_description_eng')->where('tb_news_categories.cat_slug', 'produktion')->where('tb_news_categories.cat_status', 1)->where('tb_post_articles.publish_date', '<=', $curdate)->where('tb_post_articles.status', 1)->orderBy('tb_post_articles.publish_date', 'desc')->orderBy('tb_post_articles.position_num', 'asc')->get();

                    $this->data['newspage_cat_projekten'] = \DB::table('tb_news_categories')->join('tb_post_articles', 'tb_post_articles.cat_id', '=', 'tb_news_categories.cat_id')->join('tb_users', 'tb_users.id', '=', 'tb_post_articles.user_id')->select('tb_post_articles.*', 'tb_users.first_name', 'tb_users.last_name', 'tb_news_categories.cat_title', 'tb_news_categories.cat_description', 'tb_news_categories.cat_title_eng', 'tb_news_categories.cat_description_eng')->where('tb_news_categories.cat_slug', 'Projekte')->where('tb_news_categories.cat_status', 1)->where('tb_post_articles.publish_date', '<=', $curdate)->where('tb_post_articles.status', 1)->orderBy('tb_post_articles.publish_date', 'desc')->orderBy('tb_post_articles.position_num', 'asc')->get();

                    $this->data['newspage_cat_presse'] = \DB::table('tb_news_categories')->join('tb_post_articles', 'tb_post_articles.cat_id', '=', 'tb_news_categories.cat_id')->join('tb_users', 'tb_users.id', '=', 'tb_post_articles.user_id')->select('tb_post_articles.*', 'tb_users.first_name', 'tb_users.last_name', 'tb_news_categories.cat_title', 'tb_news_categories.cat_description', 'tb_news_categories.cat_title_eng', 'tb_news_categories.cat_description_eng')->where('tb_news_categories.cat_slug', 'presse')->where('tb_news_categories.cat_status', 1)->where('tb_post_articles.publish_date', '<=', $curdate)->where('tb_post_articles.status', 1)->orderBy('tb_post_articles.publish_date', 'desc')->orderBy('tb_post_articles.position_num', 'asc')->get();

                    $this->data['newspage_cat_uberjanua'] = \DB::table('tb_news_categories')->join('tb_post_articles', 'tb_post_articles.cat_id', '=', 'tb_news_categories.cat_id')->join('tb_users', 'tb_users.id', '=', 'tb_post_articles.user_id')->select('tb_post_articles.*', 'tb_users.first_name', 'tb_users.last_name', 'tb_news_categories.cat_title', 'tb_news_categories.cat_description', 'tb_news_categories.cat_title_eng', 'tb_news_categories.cat_description_eng')->where('tb_news_categories.cat_slug', 'uber')->where('tb_news_categories.cat_status', 1)->where('tb_post_articles.publish_date', '<=', $curdate)->where('tb_post_articles.status', 1)->orderBy('tb_post_articles.publish_date', 'desc')->orderBy('tb_post_articles.position_num', 'asc')->first();

                    $this->data['newspage_cat_Videos'] = \DB::table('tb_news_categories')->join('tb_post_articles', 'tb_post_articles.cat_id', '=', 'tb_news_categories.cat_id')->join('tb_users', 'tb_users.id', '=', 'tb_post_articles.user_id')->select('tb_post_articles.*', 'tb_users.first_name', 'tb_users.last_name', 'tb_news_categories.cat_title', 'tb_news_categories.cat_description', 'tb_news_categories.cat_title_eng', 'tb_news_categories.cat_description_eng')->where('tb_news_categories.cat_slug', 'video')->where('tb_news_categories.cat_status', 1)->where('tb_post_articles.publish_date', '<=', $curdate)->where('tb_post_articles.status', 1)->orderBy('tb_post_articles.publish_date', 'desc')->orderBy('tb_post_articles.position_num', 'asc')->get();

                    $servicesArr = array('newsletter', 'presse');
                    $this->data['newspage_cat_services'] = \DB::table('tb_news_categories')->join('tb_post_articles', 'tb_post_articles.cat_id', '=', 'tb_news_categories.cat_id')->join('tb_users', 'tb_users.id', '=', 'tb_post_articles.user_id')->select('tb_post_articles.*', 'tb_users.first_name', 'tb_users.last_name')->whereIn('tb_news_categories.cat_slug', $servicesArr)->where('tb_news_categories.cat_status', 1)->where('tb_post_articles.publish_date', '<=', $curdate)->where('tb_post_articles.status', 1)->orderBy('tb_post_articles.publish_date', 'desc')->orderBy('tb_post_articles.position_num', 'asc')->get();


                    $this->data['presse_slider'] = \DB::table('tb_sliders')->where('slider_category', 'Presse')->get();
                    $this->data['presse_data'] = \DB::table('tb_presse')->first();

                    /* - - - - - -- firme section  - - - -- -  */
                    $firme_folder = \DB::table('tb_container')->where('name', 'corporate-identity')->first();
                    $firme_temp_array = array();
                    if (!empty($firme_folder)) {
                        $this->data['firme_folder'] = $firme_folder;
                        $firme_file_query = \DB::table('tb_container_files')->where('folder_id', $firme_folder->id)->get();

                        if (!empty($firme_file_query)) {
                            $f = 0;
                            foreach ($firme_file_query as $firme_file_obj) {
                                $firme_extype = explode('/', $firme_file_obj->file_type);
                                if ($firme_extype[0] == "image") {
                                    $firme_temp_array[$f]['id'] = $firme_file_obj->id;
                                    $firme_temp_array[$f]['folder_id'] = $firme_file_obj->folder_id;
                                    $firme_temp_array[$f]['name'] = ($firme_file_obj->file_display_name != '') ? $firme_file_obj->file_display_name : $firme_file_obj->file_name;
                                    $firme_temp_array[$f]['file_type'] = 'file';
                                    $firme_temp_array[$f]['cover_img'] = $firme_file_obj->file_name;
                                    $firme_temp_array[$f]['title'] = $firme_file_obj->file_title;
                                    $firme_temp_array[$f]['description'] = $firme_file_obj->file_description;
                                    $firme_temp_array[$f]['created'] = $firme_file_obj->created;
                                    $firme_temp_array[$f]['user_id'] = $firme_file_obj->user_id;
                                    $firme_temp_array[$f]['sort_num'] = $firme_file_obj->file_sort_num;
                                }
                                $f++;
                            }
                        }
                    }
                    $this->data['firme_temp_array'] = $firme_temp_array;

                    /* --- -- -  redaktionelle section - - -- - - - */
                    $this->data['presse_final_folders'] = array();
                    $pressegroup_folder = \DB::table('tb_container')->where('name', 'redaktionelle-bilder-und-texte')->first();
                    //echo '<pre>';
                    if (!empty($pressegroup_folder)) {
                        //print_r($pressegroup_folder);
                        $pre_folder = $pressegroup_folder->id;
                        $presse_parent_query = \DB::table('tb_container')->where('parent_id', $pre_folder)->get();
                        $this->data['presse_enabled_parent_array'] = array();
                        $presse_temp_arr = array();
                        $presse_temp_arr[$pre_folder]['data'] = $pressegroup_folder;
                        $presse_temp_arr[$pre_folder]['parents'] = $pressegroup_folder->parent_id;

                        $presse_is_enabled = false;
                        $presse_enabled_cnt = \DB::table('tb_frontend_container')->where('container_id', $pre_folder)->where('container_type', 'folder')->count();
                        if ($presse_enabled_cnt > 0) {
                            $presse_is_enabled = true;
                        }

                        $presse_temp_arr[$pre_folder]['is_enabled'] = $presse_is_enabled;
                        if (!empty($presse_parent_query)) {

                            foreach ($presse_parent_query as $presse_parent_obj) {
                                $presse_temp_array = array();
                                $presse_temp_array['data']['id'] = $presse_parent_obj->id;
                                $presse_temp_array['data']['folder_id'] = $presse_parent_obj->id;
                                $presse_temp_array['data']['name'] = $presse_parent_obj->display_name;
                                $presse_temp_array['data']['file_type'] = 'folder';
                                $presse_temp_array['data']['cover_img'] = $presse_parent_obj->cover_img;
                                $presse_temp_array['data']['title'] = $presse_parent_obj->title;
                                $presse_temp_array['data']['description'] = $presse_parent_obj->description;
                                $presse_temp_array['data']['created'] = $presse_parent_obj->created;
                                $presse_temp_array['data']['user_id'] = $presse_parent_obj->user_id;
                                $presse_temp_array['data']['sort_num'] = $presse_parent_obj->sort_num;

                                //$temp_array['data'] = $parent_obj;
                                $presse_temp_array['parents'] = $pressegroup_folder->parent_id . ',' . $presse_parent_obj->parent_id;
                                $presse_is_enabled = false;
                                $presse_enabled_cnt = \DB::table('tb_frontend_container')->where('container_id', $presse_parent_obj->id)->where('container_type', 'folder')->count();

                                $jpg_folder = \DB::table('tb_container')->where('parent_id', $presse_parent_obj->id)->where('name', 'jpg')->first();

                                if (!empty($jpg_folder)) {
                                    $presse_enabled_file_cnt = \DB::table('tb_container_files')->join('tb_frontend_container', 'tb_frontend_container.container_id', '=', 'tb_container_files.id')->where('tb_container_files.folder_id', $jpg_folder->id)->count();
                                    if (($presse_enabled_cnt > 0) && ($presse_enabled_file_cnt > 0)) {
                                        $presse_is_enabled = true;
                                        $this->set_all_enabled_parent($presse_temp_array['parents'] . ',' . $presse_parent_obj->id);
                                    }
                                }

                                $presse_temp_array['is_enabled'] = $presse_is_enabled;
                                $presse_child_cnt = \DB::table('tb_container')->where('parent_id', $presse_parent_obj->id)->count();
                                if ($presse_child_cnt > 0) {
                                    $presse_temp_array['child'] = $this->get_all_child_folder($presse_parent_obj->id, $presse_temp_array['parents']);
                                }


                                $presse_temp_arr[$pre_folder]['child'][$presse_parent_obj->id] = $presse_temp_array;

                                $presse_file_query = \DB::table('tb_container_files')->where('folder_id', $jpg_folder->id)->get();
                                foreach ($presse_file_query as $presse_file_obj) {
                                    $presse_temp_array = array();
                                    $presse_extype = explode('/', $presse_file_obj->file_type);
                                    if ($presse_extype[0] == "image") {
                                        $presse_enabled_cntfile = \DB::table('tb_frontend_container')->where('container_id', $presse_file_obj->id)->where('container_type', 'file')->count();
                                        if ($presse_enabled_cntfile > 0) {
                                            $presse_temp_array['data']['id'] = $presse_file_obj->id;
                                            $presse_temp_array['data']['folder_id'] = $presse_file_obj->folder_id;
                                            $presse_temp_array['data']['name'] = ($presse_file_obj->file_display_name != '') ? $presse_file_obj->file_display_name : $presse_file_obj->file_name;
                                            $presse_temp_array['data']['file_type'] = 'file';
                                            $presse_temp_array['data']['cover_img'] = $presse_file_obj->file_name;
                                            $presse_temp_array['data']['title'] = $presse_file_obj->file_title;
                                            $presse_temp_array['data']['description'] = $presse_file_obj->file_description;
                                            $presse_temp_array['data']['created'] = $presse_file_obj->created;
                                            $presse_temp_array['data']['user_id'] = $presse_file_obj->user_id;
                                            $presse_temp_array['data']['sort_num'] = $presse_file_obj->file_sort_num;

                                            $presse_temp_array['parents'] = $pressegroup_folder->parent_id . ',' . $presse_file_obj->folder_id;
                                            $presse_is_enabled = false;
                                            $presse_enabled_cnt = \DB::table('tb_frontend_container')->where('container_id', $presse_file_obj->id)->where('container_type', 'file')->count();
                                            if ($presse_enabled_cnt > 0) {
                                                $presse_is_enabled = true;
                                                $this->set_all_enabled_parent($presse_temp_array['parents'] . ',i-' . $presse_file_obj->id);
                                            }

                                            $presse_temp_array['is_enabled'] = $presse_is_enabled;

                                            $presse_temp_arr[$pre_folder]['child'][$presse_parent_obj->id]['subchild'][$presse_file_obj->id] = $presse_temp_array;
                                        }
                                    }
                                }
                            }
                        ...
                        $presse_file_query = \DB::table('tb_container_files')->where('folder_id', $pre_folder)->get();
                        foreach ($presse_file_query as $presse_file_obj) {
                            $presse_temp_array = array();
                            $presse_extype = explode('/', $presse_file_obj->file_type);
                            if ($presse_extype[0] == "image") {
                            ...
                            $presse_temp_arr[$pre_folder]['child']['i-' . $presse_file_obj->id] = $presse_temp_array;
                        }

                        $this->data['presse_final_folders'] = $this->set_parents_enables($presse_temp_arr);
                        /* print "<pre>";
                          print_r($this->data['presse_final_folders']); */
                    }


                    /* --- -- -  Weitere Themen/Downloads section - - -- - - - */
                    $this->data['weitere_final_folders'] = array();
                    $weiteregroup_folder = \DB::table('tb_container')->where('name', 'parent-placeholder')->first();
                    //echo '<pre>';
                    if (!empty($weiteregroup_folder)) {
                        //print_r($pressegroup_folder);
                        $weitere_folder = $weiteregroup_folder->id;
                        $weitere_parent_query = \DB::table('tb_container')->where('parent_id', $weitere_folder)->get();
                        $this->data['weitere_enabled_parent_array'] = array();
                        $weitere_temp_arr = array();
                        $weitere_temp_arr[$weitere_folder]['data'] = $weiteregroup_folder;
                        $weitere_temp_arr[$weitere_folder]['parents'] = $weiteregroup_folder->parent_id;

                        $weitere_is_enabled = false;
                        $weitere_enabled_cnt = \DB::table('tb_frontend_container')->where('container_id', $weitere_folder)->where('container_type', 'folder')->count();
                        if ($weitere_enabled_cnt > 0) {
                            $weitere_is_enabled = true;
                        }

                        $weitere_temp_arr[$weitere_folder]['is_enabled'] = $weitere_is_enabled;
                        if (!empty($weitere_parent_query)) {

                            foreach ($weitere_parent_query as $weitere_parent_obj) {
                                $weitere_temp_array = array();
                                $weitere_temp_array['data']['id'] = $weitere_parent_obj->id;
                                $weitere_temp_array['data']['folder_id'] = $weitere_parent_obj->id;
                                $weitere_temp_array['data']['name'] = $weitere_parent_obj->display_name;
                                $weitere_temp_array['data']['file_type'] = 'folder';
                                $weitere_temp_array['data']['cover_img'] = $weitere_parent_obj->cover_img;
                                $weitere_temp_array['data']['title'] = $weitere_parent_obj->title;
                                $weitere_temp_array['data']['description'] = $weitere_parent_obj->description;
                                $weitere_temp_array['data']['created'] = $weitere_parent_obj->created;
                                $weitere_temp_array['data']['user_id'] = $weitere_parent_obj->user_id;
                                $weitere_temp_array['data']['sort_num'] = $weitere_parent_obj->sort_num;

                                //$temp_array['data'] = $parent_obj;
                                $weitere_temp_array['parents'] = $weiteregroup_folder->parent_id . ',' . $weitere_parent_obj->parent_id;
                                $weitere_is_enabled = false;
                                $weitere_enabled_cnt = \DB::table('tb_frontend_container')->where('container_id', $weitere_parent_obj->id)->where('container_type', 'folder')->count();

                                $weiterejpg_folder = \DB::table('tb_container')->where('parent_id', $weitere_parent_obj->id)->where('name', 'jpg')->first();

                                if (!empty($weiterejpg_folder)) {
                                    $weitere_enabled_file_cnt = \DB::table('tb_container_files')->join('tb_frontend_container', 'tb_frontend_container.container_id', '=', 'tb_container_files.id')->where('tb_container_files.folder_id', $weiterejpg_folder->id)->count();
                                    if (($weitere_enabled_cnt > 0) && ($weitere_enabled_file_cnt > 0)) {
                                        $weitere_is_enabled = true;
                                        $this->set_all_enabled_parent($weitere_temp_array['parents'] . ',' . $weitere_parent_obj->id);
                                    }
                                }

                                $weitere_temp_array['is_enabled'] = $weitere_is_enabled;
                                $weitere_child_cnt = \DB::table('tb_container')->where('parent_id', $weitere_parent_obj->id)->count();
                                if ($weitere_child_cnt > 0) {
                                    $weitere_temp_array['child'] = $this->get_all_child_folder($weitere_parent_obj->id, $weitere_temp_array['parents']);
                                }


                                $weitere_temp_arr[$pre_folder]['child'][$weitere_parent_obj->id] = $weitere_temp_array;

                                $weitere_file_query = \DB::table('tb_container_files')->where('folder_id', $weiterejpg_folder->id)->get();
                                foreach ($weitere_file_query as $weitere_file_obj) {
                                    $weitere_temp_array = array();
                                    $weitere_extype = explode('/', $weitere_file_obj->file_type);
                                    if ($weitere_extype[0] == "image") {
                                        $weitere_enabled_cntfile = \DB::table('tb_frontend_container')->where('container_id', $weitere_file_obj->id)->where('container_type', 'file')->count();
                                        if ($weitere_enabled_cntfile > 0) {
                                            $weitere_temp_array['data']['id'] = $weitere_file_obj->id;
                                            $weitere_temp_array['data']['folder_id'] = $weitere_file_obj->folder_id;
                                            $weitere_temp_array['data']['name'] = ($weitere_file_obj->file_display_name != '') ? $weitere_file_obj->file_display_name : $weitere_file_obj->file_name;
                                            $weitere_temp_array['data']['file_type'] = 'file';
                                            $weitere_temp_array['data']['cover_img'] = $weitere_file_obj->file_name;
                                            $weitere_temp_array['data']['title'] = $weitere_file_obj->file_title;
                                            $weitere_temp_array['data']['description'] = $weitere_file_obj->file_description;
                                            $weitere_temp_array['data']['created'] = $weitere_file_obj->created;
                                            $weitere_temp_array['data']['user_id'] = $weitere_file_obj->user_id;
                                            $weitere_temp_array['data']['sort_num'] = $weitere_file_obj->file_sort_num;

                                            $weitere_temp_array['parents'] = $weiteregroup_folder->parent_id . ',' . $weitere_file_obj->folder_id;
                                            $weitere_is_enabled = false;
                                            $weitere_enabled_cnt = \DB::table('tb_frontend_container')->where('container_id', $weitere_file_obj->id)->where('container_type', 'file')->count();
                                            if ($weitere_enabled_cnt > 0) {
                                                $weitere_is_enabled = true;
                                                $this->set_all_enabled_parent($weitere_temp_array['parents'] . ',i-' . $weitere_file_obj->id);
                                            }

                                            $weitere_temp_array['is_enabled'] = $weitere_is_enabled;

                                            $weitere_temp_arr[$weitere_folder]['child'][$weitere_parent_obj->id]['subchild'][$weitere_file_obj->id] = $weitere_temp_array;
                                        }
                                    }
                                }
                            }
                        ...
                        $weitere_file_query = \DB::table('tb_container_files')->where('folder_id', $weitere_folder)->get();
                        foreach ($weitere_file_query as $weitere_file_obj) {
                            $weitere_temp_array = array();
                            $weitere_extype = explode('/', $weitere_file_obj->file_type);
                            if ($weitere_extype[0] == "image") {
                            ...
                            $weitere_temp_arr[$pre_folder]['child']['i-' . $weitere_file_obj->id] = $weitere_temp_array;
                        }

                        $this->data['weitere_final_folders'] = $this->set_parents_enables($weitere_temp_arr);
                    }


                    $this->data['downloads_slider'] = \DB::table('tb_sliders')->where('slider_category', 'Downloads')->get();
                    $this->data['produktinformation'] = \DB::table('tb_settings')->where('key_value', 'produktinformation')->first();

                    /* --- -- -  Publikationen section downloads page - - -- - - - */
                    $this->data['publikationen_final_folders'] = array();
                    $publikationengroup_folder = \DB::table('tb_container')->where('name', 'publikationen')->first();
                    //echo '<pre>';
                    if (!empty($publikationengroup_folder)) {
                        //print_r($pressegroup_folder);
                        $publikationen_folder = $publikationengroup_folder->id;
                        $publikationen_parent_query = \DB::table('tb_container')->where('parent_id', $publikationen_folder)->get();
                        $this->data['publikationen_enabled_parent_array'] = array();
                        $publikationen_temp_arr = array();
                        $publikationen_temp_arr[$publikationen_folder]['data'] = $publikationengroup_folder;
                        $publikationen_temp_arr[$publikationen_folder]['parents'] = $publikationengroup_folder->parent_id;

                        $publikationen_is_enabled = false;
                        $publikationen_enabled_cnt = \DB::table('tb_frontend_container')->where('container_id', $publikationen_folder)->where('container_type', 'folder')->count();
                        if ($publikationen_enabled_cnt > 0) {
                            $publikationen_is_enabled = true;
                        }

                        $publikationen_temp_arr[$publikationen_folder]['is_enabled'] = $publikationen_is_enabled;
                        if (!empty($publikationen_parent_query)) {
                            foreach ($publikationen_parent_query as $publikationen_parent_obj) {
                                $publikationen_temp_array = array();
                                $publikationen_temp_array['data']['id'] = $publikationen_parent_obj->id;
                                $publikationen_temp_array['data']['folder_id'] = $publikationen_parent_obj->id;
                                $publikationen_temp_array['data']['name'] = (\Session::get('newlang') == 'English') ? $publikationen_parent_obj->display_name_eng : $publikationen_parent_obj->display_name;
                                $publikationen_temp_array['data']['file_type'] = 'folder';
                                $publikationen_temp_array['data']['cover_img'] = $publikationen_parent_obj->cover_img;
                                $publikationen_temp_array['data']['title'] = (\Session::get('newlang') == 'English') ? $publikationen_parent_obj->title_eng : $publikationen_parent_obj->title;
                                $publikationen_temp_array['data']['description'] = (\Session::get('newlang') == 'English') ? $publikationen_parent_obj->description_eng : $publikationen_parent_obj->description;
                                $publikationen_temp_array['data']['created'] = $publikationen_parent_obj->created;
                                $publikationen_temp_array['data']['user_id'] = $publikationen_parent_obj->user_id;
                                $publikationen_temp_array['data']['sort_num'] = $publikationen_parent_obj->sort_num;
                                $publikationen_temp_array['data']['imgsrc'] = '';
                                $publikationen_temp_array['data']['pdfimg'] = '';

                                //$temp_array['data'] = $parent_obj;
                                $publikationen_temp_array['parents'] = $publikationengroup_folder->parent_id . ',' . $publikationen_parent_obj->parent_id;
                                $publikationen_is_enabled = false;
                                $publikationen_enabled_cnt = \DB::table('tb_frontend_container')->where('container_id', $publikationen_parent_obj->id)->where('container_type', 'folder')->count();

                                $publikationen_enabled_file_cnt = \DB::table('tb_container_files')->join('tb_frontend_container', 'tb_frontend_container.container_id', '=', 'tb_container_files.id')->where('tb_container_files.folder_id', $publikationen_folder)->count();
                                if (($publikationen_enabled_cnt > 0) && ($publikationen_enabled_file_cnt > 0)) {
                                    $publikationen_is_enabled = true;
                                    $this->set_all_enabled_parent($publikationen_temp_array['parents'] . ',' . $publikationen_parent_obj->id);
                                }

                                $publikationen_temp_array['is_enabled'] = $publikationen_is_enabled;
                                $publikationen_child_cnt = \DB::table('tb_container')->where('parent_id', $publikationen_parent_obj->id)->count();
                                if ($publikationen_child_cnt > 0) {
                                    $publikationen_temp_array['child'] = $this->get_all_child_folder($publikationen_parent_obj->id, $publikationen_temp_array['parents']);
                                }


                                $publikationen_temp_arr[$publikationen_folder]['child'][$publikationen_parent_obj->id] = $publikationen_temp_array;

                                $publikationen_file_query = \DB::table('tb_container_files')->where('folder_id', $publikationen_parent_obj->id)->get();

                                foreach ($publikationen_file_query as $publikationen_file_obj) {
                                    $publikationen_temp_array = array();
                                    $publikationen_enabled_cntfile = \DB::table('tb_frontend_container')->where('container_id', $publikationen_file_obj->id)->where('container_type', 'file')->count();
                                    if ($publikationen_enabled_cntfile > 0) {
                                        $publikationen_temp_array['data']['id'] = $publikationen_file_obj->id;
                                        $publikationen_temp_array['data']['folder_id'] = $publikationen_file_obj->folder_id;
                                        $publikationen_temp_array['data']['name'] = (\Session::get('newlang') == 'English') ? ($publikationen_file_obj->file_display_name_eng != '') ? $publikationen_file_obj->file_display_name_eng : $publikationen_file_obj->file_name : ($publikationen_file_obj->file_display_name != '') ? $publikationen_file_obj->file_display_name : $publikationen_file_obj->file_name;
                                        $publikationen_temp_array['data']['file_type'] = 'file';
                                        $publikationen_temp_array['data']['cover_img'] = $publikationen_file_obj->file_name;
                                        $publikationen_temp_array['data']['title'] = (\Session::get('newlang') == 'English') ? $publikationen_file_obj->file_title_eng : $publikationen_file_obj->file_title;
                                        $publikationen_temp_array['data']['description'] = (\Session::get('newlang') == 'English') ? $publikationen_file_obj->file_description_eng : $publikationen_file_obj->file_description;
                                        $publikationen_temp_array['data']['created'] = $publikationen_file_obj->created;
                                        $publikationen_temp_array['data']['user_id'] = $publikationen_file_obj->user_id;
                                        $publikationen_temp_array['data']['sort_num'] = $publikationen_file_obj->file_sort_num;
                                        $publikationen_temp_array['data']['imgsrc'] = (new ContainerController)->getThumbpath($publikationen_file_obj->folder_id);
                                        $publikationen_temp_array['data']['pdfimg'] = '';

                                        $check_pdfimg = \DB::table('td_pdf_images')->where('file_id', $publikationen_file_obj->id)->first();
                                        if (!empty($check_pdfimg)) {
                                            $publikationen_temp_array['data']['pdfimg'] = $check_pdfimg->image_name;
                                        }

                                        $publikationen_temp_array['parents'] = $publikationengroup_folder->parent_id . ',' . $publikationen_file_obj->folder_id;
                                        $publikationen_is_enabled = false;
                                        $publikationen_enabled_cnt = \DB::table('tb_frontend_container')->where('container_id', $publikationen_file_obj->id)->where('container_type', 'file')->count();
                                        if ($publikationen_enabled_cnt > 0) {
                                            $publikationen_is_enabled = true;
                                            $this->set_all_enabled_parent($publikationen_temp_array['parents'] . ',i-' . $publikationen_file_obj->id);
                                        }

                                        $publikationen_temp_array['is_enabled'] = $publikationen_is_enabled;

                                        $publikationen_temp_arr[$publikationen_folder]['child'][$publikationen_parent_obj->id]['subchild'][$publikationen_file_obj->id] = $publikationen_temp_array;
                                    }
                                }
                            }
                        }

                        $publikationen_file_query = \DB::table('tb_container_files')->where('folder_id', $publikationen_folder)->get();
                        foreach ($publikationen_file_query as $publikationen_file_obj) {
                            $publikationen_temp_array = array();
                            $publikationen_extype = explode('/', $publikationen_file_obj->file_type);
                            $publikationen_temp_array['data']['id'] = $publikationen_file_obj->id;
                            $publikationen_temp_array['data']['folder_id'] = $publikationen_file_obj->folder_id;
                            $publikationen_temp_array['data']['name'] = ($publikationen_file_obj->file_display_name != '') ? $publikationen_file_obj->file_display_name : $publikationen_file_obj->file_name;
                            $publikationen_temp_array['data']['file_type'] = 'file';
                            $publikationen_temp_array['data']['cover_img'] = $publikationen_file_obj->file_name;
                            $publikationen_temp_array['data']['title'] = $publikationen_file_obj->file_title;
                            $publikationen_temp_array['data']['description'] = $publikationen_file_obj->file_description;
                            $publikationen_temp_array['data']['created'] = $publikationen_file_obj->created;
                            $publikationen_temp_array['data']['user_id'] = $publikationen_file_obj->user_id;
                            $publikationen_temp_array['data']['sort_num'] = $publikationen_file_obj->file_sort_num;
                            $publikationen_temp_array['data']['imgsrc'] = (new ContainerController)->getThumbpath($publikationen_file_obj->folder_id);
                            ...
                            $publikationen_temp_arr[$publikationen_folder]['child']['i-' . $publikationen_file_obj->id] = $publikationen_temp_array;
                        }

                        $this->data['publikationen_final_folders'] = $publikationen_temp_arr;

                        $downloads_file_query = \DB::table('tb_container_files')->where('folder_id', $publikationengroup_folder->id)->orderBy('file_sort_num', 'asc')->first();
                        if (!empty($downloads_file_query)) {
                            $this->data['landing_downlaods_file'] = $downloads_file_query;
                        }
                    }

                    // Musterbestellung and Pflegehinweise page start

                    $this->data['musterbestellung_slider'] = \DB::table('tb_sliders')->where('slider_category', 'Musterbestellung')->get();

                    $this->data['pflegehinweise_slider'] = \DB::table('tb_sliders')->where('slider_category', 'Pflegehinweise')->get();

                    $shop_cat_Arr = \DB::table('tb_shop_product_categories')->where('cat_status', 1)->get();
                    $shopArr = array();
                    if (!empty($shop_cat_Arr)) {
                        foreach ($shop_cat_Arr as $shop_cat) {
                            $shopArr[$shop_cat->page][$shop_cat->id]['data'] = $shop_cat;
                            $shop_cat_detail = \DB::table('tb_shop')->where('page', $shop_cat->page)->where('product_cat_id', $shop_cat->id)->where('product_status', 1)->get();
                            $shopArr[$shop_cat->page][$shop_cat->id]['detail'] = array();
                            if (!empty($shop_cat_detail)) {
                                foreach ($shop_cat_detail as $cat_detail) {
                                    $shopArr[$shop_cat->page][$shop_cat->id]['detail'] = $cat_detail;
                                    $shop_products = \DB::table('tb_shop_products')->where('shop_id', $cat_detail->id)->where('cat_id', $cat_detail->product_cat_id)->get();
                                    $shopArr[$shop_cat->page][$shop_cat->id]['products'] = array();
                                    if (!empty($shop_products)) {
                                        foreach ($shop_products as $product) {
                                            $shopArr[$shop_cat->page][$shop_cat->id]['products'][$product->id] = $product;
                                        }
                                    }
                                }
                            }
                    ...
                    if (Input::get('s', false)) {
                        ...
                        if (isset($TagsConId)) {
                            ...
                            $props = \DB::table('tb_properties')->where('property_name', [$container_names])->get();
                        }
                    } else {
                        $props = \DB::table('tb_properties')->get();
                    }
...
//                            $seaprops = \DB::table('tb_properties')->where('property_name', 'like', '%' . $keyword . '%')->where('property_status', 1)->where('tb_properties.property_type', 'Hotel')->get();
                            ...
                            $query .= "ORDER BY (SELECT rack_rate FROM tb_properties_category_rooms_price WHERE tb_properties_category_rooms_price.property_id = tb_properties.id ORDER BY rack_rate DESC LIMIT 1) * 1 DESC ";
                        ...
                        if (!empty($cateObj)) {
                            ...
                            if ($arrive != '') {
                                ...
                                $catprops = \DB::select(DB::raw("SELECT * FROM tb_properties JOIN tb_properties_category_rooms ON tb_properties_category_rooms.property_id = tb_properties.id WHERE tb_properties.property_status='1' AND tb_properties_category_rooms.room_active_from <= '$arrive' $getdestind  $getcats "));
                            } else {
                                $catprops = \DB::select(DB::raw("SELECT * FROM tb_properties WHERE property_status='1' $getcats "));
                            }
                            if (!empty($catprops)) {
...
//                        });
    ...
    public function getPropertyDetail(Request $request) {
		...
		$this->data['sidebardetailAds'] = \DB::table('tb_advertisement')->where('adv_type', 'sidebar')->where('adv_position', 'detail')->get();
...
//        });
    ...
    public function getPropertyByCategoryDestination(Request $request) {
        ...
        if (isset($request->refine_search) && strtolower($request->cat) == 'yachts') {
            ...
            $query = "SELECT id, property_name, property_slug, yacht_for_charter "
                    . "FROM tb_properties "
        ...
        if (!is_null($request->area) && $request->area == 'country') {
