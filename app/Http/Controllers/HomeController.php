            ...
            if (true) {
                if (count($content) >= 1) {
                    ...
                    if (file_exists($filename)) {
                        ...
                        $orderBy = "ORDER BY (SELECT rack_rate FROM tb_properties_category_rooms_price pcrp, tb_properties pr WHERE pcrp.property_id = pr.id ORDER BY rack_rate DESC LIMIT 1) * 1 DESC, pr.editor_choice_property desc, pr.feature_property desc ";
                        $limit = " LIMIT ". $pageStart.",".$perPage; 
