...
use App\Models\Lightboxorders;
use Illuminate\Http\Request;
use Illuminate\Pagination\LengthAwarePaginator as Paginator;
use Validator, Input, Redirect ; 


class LightboxordersController extends Controller {

	protected $layout = "layouts.main";
	protected $data = array();	
	public $module = 'lightboxorders';
	static $per_page	= '10';
	...
	public function getIndex( Request $request )
	{
		...
		$pagination->setPath('lightboxorders');
		...
		return view('lightboxorders.index',$this->data);
	}	
	...
	function getUpdate(Request $request, $id = null)
	{
		...
		$this->data['id'] = $id;
		return view('lightboxorders.form',$this->data);
	}	
	...
	public function getShow( $id = null)
	{
		...
		return view('lightboxorders.view',$this->data);	
	}	
	...
	function postSave( Request $request)
	{
		...
		if ($validator->passes()) {
			$data = $this->validatePost('tb_lightboxorders');
				
			$id = $this->model->insertRow($data , $request->input('id'));
			
			if(!is_null($request->input('apply')))
			{
				$return = 'lightboxorders/update/'.$id.'?return='.self::returnUrl();
			} else {
				$return = 'lightboxorders?return='.self::returnUrl();
			}
			...
			return Redirect::to('lightboxorders/update/'.$id)->with('messagetext',\Lang::get('core.note_error'))->with('msgstatus','error')
	...
	public function postDelete( Request $request)
	{
		...
		if(count($request->input('ids')) >=1)
		{
			...
			// redirect
			return Redirect::to('lightboxorders')
