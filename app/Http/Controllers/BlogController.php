...
class BlogController extends Controller {
	...
	function postSave( Request $request)
	{
		...
		if ($validator->passes()) {
			...
			$file = \DB::table('tb_blogs')->where('blogID', $id)->first();
			$destinationPath = public_path().'/uploads/images/';
			$newfilename = $file->image;
			
			if($newfilename!="")
			{
				if (! \File::exists($destinationPath.'front_'.$newfilename))
				{
					// IMage for frontend
					$ftimg = \Image::make($destinationPath.$newfilename);
					$ftimg->resize(310, 170);
					$ftimgfile = 'front_'.$newfilename;
					$ftimg->save($destinationPath.$ftimgfile);
				}
				
				if (! \File::exists($destinationPath.'news_top_'.$newfilename))
				{
					// IMage for newspage top
					$ftimg = \Image::make($destinationPath.$newfilename);
					$ftimg->resize(505, 342);
					$ftimgfile = 'news_top_'.$newfilename;
					$ftimg->save($destinationPath.$ftimgfile);
				}
				
				if (! \File::exists($destinationPath.'news_bot_'.$newfilename))
				{
					// IMage for newspage bottom
					$ftimg = \Image::make($destinationPath.$newfilename);
					$ftimg->resize(225, 165);
					$ftimgfile = 'news_bot_'.$newfilename;
					$ftimg->save($destinationPath.$ftimgfile);
				}
