...
public static function alphaID($in, $to_num = false, $pad_up = false, $passKey = null)
{
	...
	public static function showUploadedFile($file,$path , $width = 50, $height = 30, $cls = "img-circle") {
		...
		//	echo $files ;
			...
			if($info['extension'] == "jpg" || $info['extension'] == "jpeg" ||  $info['extension'] == "png" || $info['extension'] == "gif" || $info['extension'] == "JPG") 
			{
				...
				<img src="'.asset( $path_file . $file ).'" border="0" width="'. $width .'" height="'. $height .'" class="'.$cls.'" /></a></p>';
			} else {
				$path_file = str_replace("./","",$path);
			...
			return "<img src='".asset('/uploads/images/no-image.png')."' border='0' width='". $width ."' height='". $height ."' class='".$cls."' /></a>";
	...
	public static function gridDisplay($val , $field, $arr) {
		...
		if(isset($arr['valid']) && $arr['valid'] ==1)
		{
			...
			if(count($Q) >= 1 )
			{
				...
				$fields1 = (isset($fields[0]))?$fields[0]:'';
				$fields2 = (isset($fields[1]))?$fields[1]:'';
				$fields3 = (isset($fields[2]))?$fields[2]:'';
				$v= '';
				$v .= ($fields1 !='' ?  $row->$fields1.' ' : '');
				$v .= ($fields2 !=''  ? $row->$fields2.' ' : '');
				$v .= ($fields3 !=''  ? $row->$fields3.' ' : '');
	...
	public static function avatar( $width =75, $cls='img-circle')
	{
		$avatar = '<img alt="" src="http://www.gravatar.com/avatar/'.md5(Session::get('email')).'" class="'.$cls.'" width="'.$width.'" />';
		$Q = DB::table("tb_user_company_details")->where("user_id",'=',Session::get('uid'))->get();
		if(count($Q)>=1) 
		{
			$row = $Q[0];
			$files =  './uploads/users/company/'.$row->company_logo ;
			if($row->company_logo !='' ) 	
			{
				if( file_exists($files))
				{
					return  '<img src="'.asset('uploads/users/company').'/'.$row->company_logo.'" border="0" width="'.$width.'" class="'.$cls.'" />';
				} else {
					return $avatar;
				}	
			} else {
	...
	public static function avatarProfile( $width =75, $height =75, $cls='img-circle')
	{
		...
		if(count($Q)>=1) 
		{
			...
			if($row->avatar !='' ) 	
			{
				if( file_exists($files))
				{
					return  '<img src="'.asset('uploads/users').'/'.$row->avatar.'" border="0" width="'.$width.'" height="'.$height.'" class="'.$cls.'" />';
				} else {
					return $avatar;
				}	
			} else {
