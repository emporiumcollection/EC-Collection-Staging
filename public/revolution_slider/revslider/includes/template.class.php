...
class RevSliderTemplate {
		...
		if(wp_mkdir_p( $upload_dir['basedir'].$this->templates_path ) ) { //check here to not flood the server
			$request = wp_remote_post($this->templates_url.$this->templates_download, array(
				...
				'body' => array(
					...
					'uid' => urlencode($uid),
					'product' => urlencode(RevSliderGlobals::SLIDER_PRODUCT)
				),
				'timeout' => 45 //DIRK 
			));
				...
				if($response = $request['body']) {
					if($response !== 'invalid'){
						...
						if($ret !== false){
                            ...
                            $return = array('error' => __('Can\'t write the file into the uploads folder, please change permissions and try again!', 'revslider'));
						}
                    }else{
            ...
            $return = array('error' => __('Can\'t write into the uploads folder, please change permissions and try again!', 'revslider'));
		}
	...
	private function _update_images(){
		...
		$ci = &get_instance();
		$ci->load->model('curl_model', 'WP_Http_Curl');
		$curl = ($chk) ? $ci->WP_Http_Curl : false;
		...
		if(!empty($templates) && is_array($templates)){
            // true - use common media folder
			$upload_dir = wp_upload_dir(true); // Set upload folder
	...
	public function getThemePunchTemplateSliders(){
		...
		$latestVersion = get_option('revslider-latest-version', RevSliderGlobals::SLIDER_REVISION);
		$availablePlugins = array();
		foreach ((array) get_option('revslider-addons') as $pluginKey => $plugin) {
		    $availablePlugins[] = "$pluginKey/$pluginKey.php";
		}
		...
		if(!empty($sliders)){
			...
			if(!empty($defaults)){
				foreach($defaults as $key => $slider){
				    ...
				    if (isset($slider['required']) && version_compare($latestVersion, $slider['required'], '<')) {
				        unset($defaults[$key]);
				        continue;
