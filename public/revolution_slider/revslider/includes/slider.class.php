...
class RevSliderSlider extends RevSliderElementsBase{
	...
	private function createUpdateSliderFromOptions($options, $sliderID = null){
		...
		if(empty($sliderID)){	//create slider	
            ...
            do_action('revslider_slider_createUpdateSliderFromOptions', $sliderID);
			...
			$this->db->update(RevSliderGlobals::$table_sliders,$arrData,array("id"=>$sliderID));
    ...
    private function duplicateSlider($title = false, $prefix = false){
		...
		if($title === false){
            ...
            if($prefix !== false){
                ...
                $newSliderAlias = sanitize_title_with_dashes($title.' '.$params['title']);
            }else{
                $newSliderTitle = sanitize_text_field($title);
                $newSliderAlias = sanitize_title_with_dashes($title);
            }
        ...
        do_action('revslider_slider_duplicateSlider', $lastID);
	...
	private function copyMoveSlide($slideID,$targetSliderID,$operation){
        ...
        do_action('revslider_slider_copyMoveSlide', $targetSliderID);
	...
	private function shiftOrder($fromOrder){
        ...
        do_action('revslider_slider_shiftOrder', $this->id);
    }
	...
	public function createSliderFromOptions($options){
        ...
        do_action('revslider_slider_createSliderFromOptions', $sliderID);
    ...
    public function importSliderFromPost($updateAnim = true, $updateStatic = true, $exactfilepath = false, $is_template = false, $single_slide = false, $updateNavigation = true){
        ...
        try{
			...
			$mbExists = function_exists('mb_strlen');
			if ($mbExists) {
				$content = preg_replace_callback('!s:(\d+):"(.*?)";!', array('RevSliderSlider', 'clear_error_in_mb_string') , $content); //clear errors in string
			} else {
			...
			if (in_array($sliderParams['source_type'], array('posts', 'specific_posts', 'woo', 'woocommerce')))
				RevSliderFunctions::throwError("Post and Woocommerce based sliders not supported in this version. Slider not imported");
			...
			if (isset($sliderParams["background_image"]) && $sliderParams["background_image"]) {
                $_pathinfo = pathinfo($sliderParams["background_image"]);
                        ...
                        if(isset($layer['type']) && $layer['type'] == 'svg'){
                            if(isset($layer['svg']) && isset($layer['svg']->src)){
								...
								if (strpos($layer['svg']->src, '/plugins/revslider/') === 0)
									$layer['svg']->src = substr($layer['svg']->src, strlen('/plugins/'));

                                if(strpos($layer['svg']->src, '/plugins/revslider') !== false) {
                                    $layer['svg']->src = base_url() . ltrim($layer['svg']->src, '/');
                                } else {
                                    $layer['svg']->src = str_replace('/revslider/public/assets/assets/svg', '', $layer['svg']->src);
                                }
                            }
                        }
            ...
            if($is_template !== false){ //duplicate the slider now, as we just imported the "template"
				if($single_slide !== false){ //add now one Slide to the current Slider
					...
					$talias = strtolower(url_title($title));
					$ti = 1;
					while($this->isAliasExistsInDB($talias)){ //set a new alias and title if its existing in database
						$talias = strtolower(url_title($title)) . $ti;
						$ti++;
					}
	 ...
	 */
	public function updateParam($arrUpdate, $doAction = true){
		$this->validateInited();
		...
		if ( $doAction )
		    do_action('revslider_slider_updateParam', $this->id);
	}
	...
	public function updateSetting($arrUpdate){
        ...
        do_action('revslider_slider_updateSetting', $this->id);
	}
    ...
    /**
     * duplicate package from input data
     * @since: 5.2.5
     */
	...
	public function duplicateSlideFromData($data){
        ...
        do_action('revslider_slider_duplicateSlideFromData', $sliderID);
	...
	public function copySlideToSlider($data){
        ...
        do_action('revslider_slider_copySlideToSlider', $sliderID);
	...
	public function updateSlidesOrderFromData($data){
        ...
        do_action('revslider_slider_updateSlidesOrderFromData', $sliderID);
	...
	public function getStaticSlideForExport($useDummy = false){
        ...
        $arrSlidesExport = apply_filters('revslider_getSlidesForExport', $arrSlidesExport);
        $arrSlidesExport = apply_filters('revslider_getStaticSlideForExport', $arrSlidesExport);
        return $arrSlidesExport;
	}
	...
	public static function clear_error_in_mb_string($m){
		return 's:'.mb_strlen($m[2], '8bit').':"'.$m[2].'";';
	}
