...
function serve_media_shortcode($shortcode) {
    ...
    if (!$raw_data) {
        $page_res = client_request('get', '/p/' . $shortcode . '/');
            ...
            switch ($page_res['http_code']) {
                ...
                case 200:
                    ...
                    if (!preg_match('#window\._sharedData\s*=\s*(.*?)\s*;\s*</script>#', $page_res['body'], $page_data_matches)) {
                        ...
                        if (empty($page_data['entry_data']['PostPage'][0]['media'])) {
                            $result = error();

                        } else {
                            $raw_data = $page_data['entry_data']['PostPage'][0]['media'];
                        ...
                        if (!$page_data || empty($page_data['entry_data']['ProfilePage'][0]['user'])) {
                            ...
                            if ($user_data['is_private']) {
                                ...
                                if ($query_res['http_code'] != 200) {
                                    ...
                                    } else if (empty($query_data['media']['nodes'])) {
                                        $user_data['media']['nodes'] = $query_data['media']['nodes'];
                                        $raw_data = $user_data;
...
function instagram_format_media($raw_data, $external = null) {
    ...
    if (!empty($raw_data['owner']) && !$formatted_user) {
        $formatted_user = array(
            ...
            'profile_picture' => $raw_data['owner']['profile_pic_url'],
            'id' => $raw_data['owner']['id'],
            'full_name' => $raw_data['owner']['full_name']
        );
    }
    ...
    $formatted_item = array(
        ...
        'link' => 'https://www.instagram.com/p/' . $raw_data['code'] . '/',
        'likes' => null,
        'images' => array(
        ...
        'users_in_photo' => null,
        'caption' => null,
        'type' => $raw_data['is_video'] ? 'video' : 'image',
